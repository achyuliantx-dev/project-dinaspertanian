<div class="page-header">
	<h1 class="page-title">Pengelolaan Data IPW Kecamatan</h1>
				...
				<table class="table table-hover table-striped w-full" id="tbl-xdt">
					<thead>
						<tr>
							...
							<th>Nama IPW Kec</th>
							<th>Kecamatan</th>
							<th>Batas Utara Kecamatan</th>
							<<th>Batas TimurKecamatan</th>
							<th>Batas Selatan Kecamatan</th>
							<th>Batas Barat Kecamatan</th>
						</tr>
					</thead>
				</table>
			</div>
			...
			<div class="panel-body">
				<div class="form-row">
					<div class="form-group col-md-12">
						<label class="form-control-label">ID IPW Kecamatan*</label>
					...
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label">Nama IPW Kecamatan*</label>
					...
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label" style="margin-bottom: -5px;">Batas Utara Kecamatan</label>
					...
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label" style="margin-bottom: -5px;">Batas Timur Kecamatan</label>
					...
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label" style="margin-bottom: -5px;">Batas Selatan Kecamatan</label>
						<select class="form-control select2" id="cbobsk">
							<option value="">Silahkan Pilih</option>
							<?php
							if (is_array($dtkec)) {
								if (count($dtkec) > 0) {
					...
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label" style="margin-bottom: -5px;">Batas Barat Kecamatan</label>
						<select class="form-control select2" id="cbobbk">
							<option value="">Silahkan Pilih</option>
							<?php
							if (is_array($dtkec)) {
								if (count($dtkec) > 0) {
...
<script>
	...
	function tambah() {
		...
		if (id == "" || nama == "" || kec == "" || buk == ""  || btk == "" || bsk == ""|| bbk == "" ) {
			swal({
				title: 'Tambah Gagal',
			...
			success: function(x) {
				...
				if (y == 1) {
					swal({
						...
						text: 'Data IPW Kecamatan Berhasil di Tambahkan',
						icon: 'success'
					}).then((Refreshh) => {
						refresh();
						tabel.ajax.reload(null, false);
			...
			success: function(x) {
				...
				if (y == 1) {
					swal({
						...
						text: 'Data IPW Kecamatan Berhasil di Update',
						icon: 'success'
					}).then((Refreshh) => {
						refresh();
						tabel.ajax.reload(null, false);
					...
					success: function(x) {
						...
						if (y == 1) {
							swal({
								...
								text: 'Data IPW Kecamatan Berhasil di Hapus',
								icon: 'success'
							}).then((Refreshh) => {
								refresh();
								tabel.ajax.reload(null, false);
		...
		$.ajax({
            ...
            success: function(x){
				...
				if(xx[0] == 1){
					$("#txtid").val(xx[1]);
					$("#txtnama").val(xx[2]);
					$("#cbokec").val(xx[3]).change();
					$("#cbobuk").val(xx[4]).change();
					$("#cbobtk").val(xx[5]).change();
					$("#cbobsk").val(xx[6]).change();
					$("#cbobbk").val(xx[7]).change();
