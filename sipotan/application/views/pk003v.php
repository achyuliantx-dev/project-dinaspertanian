<div class="page-header">
	<h1 class="page-title">Pengelolaan Data Pupuk</h1>
			...
			<div class="panel-body">
				<div class="form-row">
                    <div class="form-group col-md-12">
						<label class="form-control-label">ID Pupuk*</label>
					...
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label">Nama Pupuk*</label>
						<input type="text" class="form-control khusus_abjad jedaobyek" id="txtnama" placeholder="Masukkan Nama Pupuk" autocomplete="off">
					</div>
...
<script>
            ...
            success: function(x){
                ...
                if(y == 1){
                    swal({
						...
						text: 'Data Pupuk Berhasil di Tambahkan',
						icon: 'success'
					}).then((Refreshh)=>{
						refresh();
						tabel.ajax.reload(null, false);
            ...
            success: function(x){
                ...
                if(y == 1){
                    swal({
						...
						text: 'Data Pupuk Berhasil di Update',
						icon: 'success'
					}).then((Refreshh)=>{
						refresh();
						tabel.ajax.reload(null, false);
	...
	function hapus(){
        ...
        if(id == ""){
            swal({title: 'Hapus Gagal', text: 'ID Pupuk Kosong !', icon: 'error'});
			$("#btnupdate").attr("disabled", false);
			$("#btnhapus").attr("disabled", false);
			$("#btnrefresh").attr("disabled", false);
            return;
					...
					success: function(x){
						...
						if(y == 1){
							swal({
								...
								text: 'Data Pupuk Berhasil di Hapus',
								icon: 'success'
							}).then((Refreshh)=>{
								refresh();
								tabel.ajax.reload(null, false);
							...
							if(y == 99){
								...
								if(y == 90){
									swal({title: 'Hapus Gagal', text: 'Data Komoditas Ini Masih digunakan dalam Data Varietas Tanaman, Sehingga Tidak Dapat di Hapus Hanya Dapat di Ubah', icon: 'error'});
									refresh();
								}else{
