<div class="page-header">
	<h1 class="page-title">Pengelolaan Data Kabupaten</h1>
			...
			<div class="panel-body">
				<div class="form-row">
                    <div class="form-group col-md-12">
						<label class="form-control-label">ID Kabupaten*</label>
						<input type="text" class="form-control khusus_angka jedaobyek" id="txtid" autocomplete="off" maxlength="4" placeholder="Masukkan ID Kabupaten">
					</div>
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label">Nama Kabupaten*</label>
						<input type="text" class="form-control khusus_abjad jedaobyek" id="txtnama" placeholder="Masukkan Nama Kabupaten" autocomplete="off" maxlength="100">
					</div>
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label" style="margin-bottom: -5px;">Propinsi</label>
						<select class="form-control select2" id="cboprov">
					...
					<div class="form-group col-md-12 jedaobyek">
						...
						<select class="form-control select2" id="cbokb">
...
<script>
	...
	function tambah(){
        ...
        $.ajax({
            ...
            method: "POST",
            data: {id: id, nama: nama, prov: prov, jenis: kb},
            cache: "false",
            success: function(x){
				swal.close();
				var y = atob(x);
                if(y == 1){
                    swal({
						...
						text: 'Data Kabupaten Berhasil di Tambahkan',
						icon: 'success'
					}).then((Refreshh)=>{
						refresh();
						tabel.ajax.reload(null, false);
            ...
            success: function(x){
                ...
                if(y == 1){
                    swal({
						...
						text: 'Data Kabupaten Berhasil di Update',
						icon: 'success'
					}).then((Refreshh)=>{
						refresh();
						tabel.ajax.reload(null, false);
					...
					success: function(x){
						...
						if(y == 1){
							swal({
								...
								text: 'Data Kabupaten Berhasil di Hapus',
								icon: 'success'
							}).then((Refreshh)=>{
								refresh();
								tabel.ajax.reload(null, false);
		...
		$.ajax({
            ...
            success: function(x){
				...
				if(xx[0] == 1){
					$("#txtid").val(xx[1]);
					$("#txtnama").val(xx[2]);
					$("#cboprov").val(xx[3]).change();
					$("#cbokb").val(xx[4]).change();
