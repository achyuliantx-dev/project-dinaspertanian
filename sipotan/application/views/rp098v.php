<div class="page-header">
	<h1 class="page-title">Pengelolaan Data Rencana Pola Tanam</h1>
			...
			<div class="panel-body">
				<div class="form-row">
                     <div class="form-group col-md-12">
						<label class="form-control-label">ID Rencana PT*</label>
						<input type="text" class="form-control khusus_abjad jedaobyek" id="txtid" placeholder="Masukkan ID" autocomplete="off" maxlength="5">
					</div>
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label">Masa Tanam *</label>
						<input type="text" class="form-control khusus_abjad jedaobyek" id="txtmt" placeholder="Masukkan Masa Tanam" autocomplete="off">
					</div>
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label" style="margin-bottom: -5px;">Varietas Tanaman</label>
						<select class="form-control select2" id="cbovar">
...
<script>
	...
	function tambah(){
        ...
        $.ajax({
            ...
            method: "POST",
            data: {id: id,mt: masa_tanam,varietas: varietas},
            cache: "false",
            success: function(x){
				swal.close();
				var y = atob(x);
                if(y == 1){
                    swal({
						...
						text: 'Data Rencana Pola Tanam Berhasil di Tambahkan',
						icon: 'success'
					}).then((Refreshh)=>{
						refresh();
						tabel.ajax.reload(null, false);
            ...
            success: function(x){
                ...
                if(y == 1){
                    swal({
						...
						text: 'Data Rencana Pola Tanam Berhasil di Update',
						icon: 'success'
					}).then((Refreshh)=>{
						refresh();
						tabel.ajax.reload(null, false);
					...
					success: function(x){
						...
						if(y == 1){
							swal({
								...
								text: 'Data Rencana Pola Tanam Berhasil di Hapus',
								icon: 'success'
							}).then((Refreshh)=>{
								refresh();
								tabel.ajax.reload(null, false);
