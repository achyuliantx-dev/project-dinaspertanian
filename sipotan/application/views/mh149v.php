<div class="page-header">
	<h1 class="page-title">Pengelolaan Data Monitoring Unsur Hara</h1>
			...
			<div class="panel-body">
				<div class="form-row">
                    <div class="form-group col-md-12">
						<label class="form-control-label">ID Monitoring Unsur Hara*</label>
					...
					<div class="form-group col-md-12 jedaobyek">
						...
						<select class="form-control select2" id="cbomt">
							<option value="">Silahkan Pilih</option>
							<?php
								if(is_array($dtpol)){
									if(count($dtpol)>0){
										foreach($dtpol as $y){
											echo "<option value='".$y->id."'>".$y->id."</option>";
										}
									}
								}
							?>
					...
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label" style="margin-bottom: -5px;">Unsur Hara</label>
						<select class="form-control select2" id="cbouh">
							<option value="">Silahkan Pilih</option>
							<?php
								if(is_array($dtuh)){
									if(count($dtuh)>0){
...
<script>
	...
	function tambah(){
        ...
        $.ajax({
            ...
            method: "POST",
            data: {id: id, nop: nop, idp: idp, idu: idu, nilai: nilai},
            cache: "false",
            success: function(x){
				swal.close();
				var y = atob(x);
		...
		$.ajax({
            ...
            success: function(x){
				...
				if(xx[0] == 1){
					...
					$("#cbomt").val(xx[3]).change();
					$("#cbouh").val(xx[4]).change();
					$("#txtnilai").val(xx[5]);
