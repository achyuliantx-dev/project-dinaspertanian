...
class Ap543 extends CI_Controller {
	public $idsc;
	public $aksesc = array();
	...
	public function tambah(){
		if($this->aksesc[0] == "1"){
			...
			$operasi = $this->Map543->tambah($nama);
			if($operasi == "1"){
				$ket = "Nama Alat Penyiangan: $nama";
				$this->Mlog->log_history("Alat Penyiangan","Tambah",$ket);
			}
	...
	public function update(){
		if($this->aksesc[1] == "1"){
			...
			$nama = trim(str_replace("'","''",$this->input->post("nama")));
			$operasi = $this->Map543->update($id,$nama);
			if($operasi == "1"){
				$ket = "ID Alat Penyiangan: $id,\nNama Alat Penyiangan: $nama";
				$this->Mlog->log_history("Alat Penyiangan","Update",$ket);
			}
			echo base64_encode($operasi);
		}else{echo base64_encode("99");}
	}
	...
	public function hapus(){
		if($this->aksesc[2] == "1"){
			$id = trim(str_replace("'","''",$this->input->post("id")));
			$td = $this->Map543->cekform($id);
			if(is_array($td)){
				if(count($td) > 0){echo base64_encode("90");}
				else{
					$dt = $this->Map543->filter($id);
					$operasi = $this->Map543->hapus($id);
					if($operasi == "1"){
						foreach ($dt as $k){
							$id = $k->id;
							$nama = $k->nama;	
						...
						$ket = "ID Alat Penyiangan: $id,\nNama Alat Penyiangan: $nama";
						$this->Mlog->log_history("Alat Penyiangan","Hapus",$ket);
					}
					echo base64_encode($operasi);
				}
