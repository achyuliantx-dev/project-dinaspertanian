...
class Tkrppg123 extends CI_Controller
{
	public $idsc;
	public $aksesc = array();
	...
	public function json()
	{
		...
		foreach ($dt as $k) {
			...
			$dtisi .= '["' . $tomboledit . '","' . $id . '","' . $petugas . '","' . $jumlah  . '","' . $id_kec . '","' . $id_tkr_kode_ipwkec . '","' . $tahun . '"],';
		}
	...
	public function filter()
	{
		...
		if (is_array($dt)) {
			if (count($dt) > 0) {
				...
				echo base64_encode("1|" . $id . "|" . $petugas . "|" . $jumlah . "|" . $id_kec . "|" . $id_tkr_kode_ipwkec . "|" . $tahun);
			} else {
	...
	public function filterdesa()
	{
		...
		$hasil = $this->Mtkrppg123->filterdesa($idkec);
	...
	{
		if ($this->aksesc[0] == "1") {
			$petugas = trim(str_replace("'", "''", $this->input->post("petugas")));
			...
			$operasi = $this->Mtkrppg123->tambah($petugas, $jumlah, $id_kec, $id_tkr_kode_ipwkec, $tahun);
			if ($operasi == "1") {
				$ket = "Petugas: $petugas,\nJumlah: $jumlah,\nID Kecamatan: $id_kec,\nIPW Kecamtan: $id_tkr_kode_ipwkec,\ntahun: $tahun";
				$this->Mlog->log_history("TKR Petugas Pendamping", "Tambah", $ket);
			}
	...
	public function update()
	{
		if ($this->aksesc[1] == "1") {
			...
			$id_tkr_kode_ipwkec = trim(str_replace("'", "''", $this->input->post("id_tkr_kode_ipwkec")));
			$tahun = trim(str_replace("'", "''", $this->input->post("tahun")));
			$operasi = $this->Mtkrppg123->update($id, $petugas, $jumlah, $id_kec, $id_tkr_kode_ipwkec, $tahun);
			if ($operasi == "1") {
				$ket = "Petugas: $petugas,\nJumlah: $jumlah,\nID Kecamatan: $id_kec,\nIPW Kecamtan: $id_tkr_kode_ipwkec,\ntahun: $tahun,";
				$this->Mlog->log_history("TKR Petugas Pendamping", "Update", $ket);
			}
			echo base64_encode($operasi);
		} else {
			echo base64_encode("99");
	...
	public function tambah_akses()
	{
		if ($this->aksesc[1] == "1") {
			$id = trim(str_replace("'", "''", $this->input->post("id")));
			$petugas = trim(str_replace("'", "''", $this->input->post("nama")));
			...
			$operasi = $this->Mtkrppg123->tambah_akses($id, $petugas, $uraian, $pass, $level, $status);
			if ($operasi == "1") {
				$ket = "ID Akses: $id,\nNama: $petugas,\nuraian: $uraian,\nPassword: $pass,\nlevel: $level,\nstatus: $status";
	...
	public function hapus()
	{
		if ($this->aksesc[2] == "1") {
			$id = trim(str_replace("'", "''", $this->input->post("id")));
			$td = $this->Mtkrppg123->cekform($id);
			if (is_array($td)) {
				if (count($td) > 0) {
					echo base64_encode("90");
				} else {
					$dt = $this->Mtkrppg123->filter($id);
					$operasi = $this->Mtkrppg123->hapus($id);
					if ($operasi == "1") {
						foreach ($dt as $k) {
							$id = $k->id;
							$petugas = $k->petugas;
						...
						$ket = "Petugas: $petugas,\nJumlah: $jumlah,\nID Kecamatan: $id_kec,\nIPW Kecamtan: $id_tkr_kode_ipwkec,\ntahun: $tahun,";
						$this->Mlog->log_history("TKR Petugas Pendamping", "Hapus", $ket);
					}
					echo base64_encode($operasi);
				}
			} else {
	...
	// 	$petugas = str_replace("fileku", $kode, $filename);
	// 	$location = "assets/excel/" . $petugas;
	...
	// 					$jab = $d[22];
	// 					if ($baris != 1) {
	// 						if ($petugas == "" || $nik == "" || $nip == "" || $gelarDep == "" || $gelarBel == "" || $tempat_lahir == "" || $tgl_lahir == "" || $jumlah == "" || $agama == "" || $jsp == "" || $sek == "" || $kec == "" || $ptt == "" || $pta == "" || $jumlah == "" || $desa == "" || $pos == "" || $keterangan == "" || $email == "" || $status == "" || $jtt == "" || $jja == "" || $jab == "") {
	// 							$gagal++;
	// 						} else {

	// 							$st = $this->Mtkrppg123->tambah(
	// 								$petugas,
	// 								$nik,
	// 								$nip,
	// 								$gelarDep,
	...
	// 		array_push($arr, [$id, $petugas, $jumlah, $jumlah, $keterangan]);
	// 	}
