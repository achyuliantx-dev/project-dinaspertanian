<?php
defined('BASEPATH') or exit('No direct script access allowed');
class Ma567 extends CI_Controller
{
	public $idsc;
	public $aksesc = array();
	function __construct()
	{
		parent::__construct();
		$idformini = "ma567";
		$this->load->model('Mma567');
		$this->load->model('Mid002');
		$this->load->model('Mkc001');
		$this->load->model('Mtn123');
		$this->load->model('Mva093');
		$this->load->model('Mop112');
		$this->load->model('Mds001');
		$data["datalogin"] = $this->Mlogin->cek_login();
		$dataform = $this->Mlogin->cek_sistem($idformini);
        if(is_array($data["datalogin"])){
			foreach($dataform as $cx){
				$idsistem_sistem = $cx->id_sistem;
			}
			$idsistem_user = array();
         	foreach ($data["datalogin"] as $dl){
				$idlevel = $dl->id_level;
				array_push($idsistem_user, $dl->id_sistem);
			}
			if(array_search($idsistem_sistem, $idsistem_user) !== false){}else{redirect(base_url());}
			$data["datamenu"] = $this->Mlogin->cek_menu($idsistem_sistem, $idlevel);
			$data["dataform"] = $this->Mlogin->cek_form($idsistem_sistem, $idlevel);
			$data["ids"] = $idsistem_sistem;
			$data["idf"] = $idformini;
			$this->idsc = $idsistem_sistem;
			$idform = array(); $akses = array();
			foreach ($data["dataform"] as $dx){
				array_push($idform, $dx->id_form);
				if($dx->id_form == $idformini){array_push($akses, $dx->akses_tambah, $dx->akses_update, $dx->akses_hapus, $dx->akses_cetak);}
			}
			if(array_search($idformini, $idform) !== false){
				$data["akses"] = $akses;
				$this->aksesc = $akses; 
			}else{redirect(base_url());}
        	$this->load->view($idsistem_sistem.'/basis', $data, true);
        }else{redirect(base_url());}
    }


	public function index()
	{
		$data["fill"] = "ma567v";
		$data["dtkec"] = $this->Mkc001->data();
		$data["dtipw"] = $this->Mid002->data();
		$data["dtta"] = $this->Mtn123->data();
		$data["dtkom"] = $this->Mva093->data();
		$data["dtopt"] = $this->Mop112->data();
		$data["dtdes"] = $this->Mds001->data();
		$this->load->view($this->idsc . '/basis', $data);
	}

	public function json()
	{
		$dtJSON = '{"data": [xxx]}';
		$dtisi = "";
		$dt = $this->Mma567->data();
		foreach ($dt as $k) {
			$id = $k->id;
			$masalah = $k->masalah;
			$komo = $k->komoditas;
			$luas = $k->luas_ha;
			$tipe = $k->tipe_masalah;
			$thn = $k->tahun;
			$kec = $k->kecamatan;
			$ipw = $k->ipw;
			$desa = $k->desa;
			$tomboledit = "<button class='btn btn-icon btn-round btn-primary' data-id='" . $id . "' onclick='filter(this)'><i class='icon wb-pencil'></i></button>";
			$dtisi .= '["' . $tomboledit . '","' . $id . '","' . $masalah . '","' . $komo . '","' . $luas . '","' . $tipe . '","' . $thn . '","' . $kec . '","' . $ipw . '","' . $desa . '"],';
		}
		$dtisifix = rtrim($dtisi, ",");
		$data = str_replace("xxx", $dtisifix, $dtJSON);
		echo $data;
	}

	public function filter()
	{
		$id = trim($this->input->post("id"));
		$dt = $this->Mma567->filter($id);
		if (is_array($dt)) {
			if (count($dt) > 0) {
				foreach ($dt as $k) {
					$id = $k->id;
					$mas = $k->id_masalah;
					$komod = $k->id_komoditas;
					$luas = $k->luas_ha;
					$tipe = $k->tipe_masalah;
					$thna = $k->id_tahun;
					$cam = $k->id_kecamatan;
					$ipwd = $k->id_ipw;
					$des = $k->id_desa;
				}
				echo base64_encode("1|" . $id . "|" . $mas . "|" . $komod . "|" . $luas . "|" . $tipe . "|" . $thna. "|" . $cam . "|" . $ipwd . "|" . $des);
			} else {
				echo base64_encode("0|");
			}
		} else {
			echo base64_encode("0|");
		}
	}

	public function tambah()
	{
		if ($this->aksesc[0] == "1") {
			$masalah = trim(str_replace("'", "''", $this->input->post("masalah")));
			$komoditas = trim(str_replace("'", "''", $this->input->post("komoditas")));
			$luas = trim(str_replace("'", "''", $this->input->post("luas")));
			$tipe = trim(str_replace("'", "''", $this->input->post("tipe")));
			$thn = trim(str_replace("'", "''", $this->input->post("thn")));
			$cam = trim(str_replace("'", "''", $this->input->post("cam")));
			$ipw = trim(str_replace("'", "''", $this->input->post("ipw")));
			$desa= trim(str_replace("'", "''", $this->input->post("desa")));
			$operasi = $this->Mma567->tambah($masalah, $komoditas, $luas, $tipe, $thn, $cam, $ipw, $desa);
			if ($operasi == "1") {
				$ket = "Id Masalah: $masalah,\nId komoditas: $komoditas,\nLuas Lahan: $luas,\nTipe Masalah: $tipe,\nTahun Anggaran: $thn,\nId Kecamatan: $cam,\nId IPW Desa: $ipw,\nId Desa: $desa";
				$this->Mlog->log_history("Masalah", "Tambah", $ket);
			}
			echo base64_encode($operasi);
		} else {
			echo base64_encode("99");
		}
	}

	public function update(){
		if ($this->aksesc[1] == "1") {
			$id = trim(str_replace("'", "''", $this->input->post("id")));
			$masalah = trim(str_replace("'", "''", $this->input->post("masalah")));
			$komoditas = trim(str_replace("'", "''", $this->input->post("komoditas")));
			$luas = trim(str_replace("'", "''", $this->input->post("luas")));
			$tipe = trim(str_replace("'", "''", $this->input->post("tipe")));
			$thn = trim(str_replace("'", "''", $this->input->post("thn")));
			$cam = trim(str_replace("'", "''", $this->input->post("cam")));
			$ipw = trim(str_replace("'", "''", $this->input->post("ipw")));
			$desa= trim(str_replace("'", "''", $this->input->post("desa")));
			$operasi = $this->Mma567->update($id, $masalah, $komoditas, $luas, $tipe, $thn, $cam, $ipw, $desa);
			if ($operasi == "1") {
				$ket = "Id Masalah: $id,\nId Jenis Masalah: $masalah,\nId komoditas: $komoditas,\nLuas Lahan: $luas,\nTipe Masalah: $tipe,\nTahun Anggaran: $thn,\nId Kecamatan: $cam,\nId IPW Desa: $ipw,\nId Desa: $desa";
				$this->Mlog->log_history("Masalah", "Update", $ket);
			}
			echo base64_encode($operasi);
		}else{echo base64_encode("99");}
	}

	public function hapus()
	{
		if ($this->aksesc[2] == "1") {
			$id = trim(str_replace("'", "''", $this->input->post("id")));
			$td = $this->Mma567->cekform($id);
			if (is_array($td)) {
				if (count($td) > 0) {
					echo base64_encode("90");
				} else {
					$dt = $this->Mma567->filter($id);
					$operasi = $this->Mma567->hapus($id);
					if ($operasi == "1") {
						foreach ($dt as $k) {
							$id = $k->id;
							$mas = $k->id_masalah;
							$komod = $k->id_komoditas;
							$luas = $k->luas_ha;
							$tipe = $k->tipe_masalah;
							$thna = $k->id_tahun;
							$cam = $k->id_kecamatan;
							$ipwd = $k->id_ipw;
							$des = $k->id_desa;
						}
						$ket = "Id Masalah: $id,\nId Jenis Masalah: $mas,\nId komoditas: $komod,\nLuas Lahan: $luas,\nTipe Masalah: $tipe,\nTahun Anggaran: $thna,\nId Kecamatan: $cam,\nId IPW Desa: $ipwd,\nId Desa: $des";
						$this->Mlog->log_history("Masalah", "Delete", $ket);
					}
					echo base64_encode($operasi);
				}
			} else {
				echo base64_encode("80");
			}
		} else {
			echo base64_encode("99");
		}
	}
}
