...
class Tkrptknds123 extends CI_Controller
{
	public $idsc;
	public $aksesc = array();
	function __construct()
	{
		...
		$idformini = "tkrptknds123";
		$this->load->model('Mtkrptknds123');
	...
	public function json()
	{
		$dtJSON = '{"data": [xxx]}';
		$dtisi = "";
		$dt = $this->Mtkrptknds123->data();
		foreach ($dt as $k) {
			$id = $k->id;
			$jenis_ternak = $k->jenis_ternak;
			$jumlah_jantan = $k->jumlah_jantan;
			$jumlah = $k->jumlah;
			...
			$dtisi .= '["' . $tomboledit . '","' . $id . '","' . $jenis_ternak . '","' . $jumlah_jantan . '","' . $jumlah . '","' . $keterangan . '","' . $tahun . '","' . $id_des . '","' . $id_kec . '","' . $id_tkr_kode_ipwdes . '"],';
		}
	...
	public function filter()
	{
		...
		$dt = $this->Mtkrptknds123->filter($id);
		if (is_array($dt)) {
			if (count($dt) > 0) {
				foreach ($dt as $k) {
					$id = $k->id;
				...
				echo base64_encode("1|" . $id . "|" . $jenis_ternak . "|" . $jumlah_jantan . "|" . $jumlah . "|" . $keterangan . "|" . $tahun . "|" . $id_des . "|" . $id_kec . "|" . $id_tkr_kode_ipwdes);
			} else {
	...
	public function filterdesa()
	{
		...
		$hasil = $this->Mtkrptknds123->filterdesa($idkec);
	...
	{
		if ($this->aksesc[0] == "1") {
			$jenis_ternak = trim(str_replace("'", "''", $this->input->post("jenis_ternak")));
			$jumlah_jantan = trim(str_replace("'", "''", $this->input->post("jumlah_jantan")));
			...
			$operasi = $this->Mtkrptknds123->tambah($jenis_ternak, $jumlah_jantan, $jumlah, $keterangan, $tahun, $id_des, $id_kec, $id_tkr_kode_ipwdes);
			if ($operasi == "1") {
				$ket = "Kode Lahan: $jenis_ternak,\nJenis Pertanian: $jumlah_jantan,\nLuas Jumlah : $jumlah,\nKeterangan: $keterangan,\ntahun: $tahun,\nID Desa: $id_des,\nID Kecamatan: $id_kec,\nIPW Desa: $id_tkr_kode_ipwdes";
				$this->Mlog->log_history("TKR Peternakan Desa", "Tambah", $ket);
			}
	...
	public function update()
	{
		if ($this->aksesc[1] == "1") {
			...
			$id_kec = trim(str_replace("'", "''", $this->input->post("id_kec")));
			$id_tkr_kode_ipwdes = trim(str_replace("'", "''", $this->input->post("id_tkr_kode_ipwdes")));
			$operasi = $this->Mtkrptknds123->update($id, $jenis_ternak, $jumlah_jantan, $jumlah, $keterangan, $tahun, $id_des, $id_kec, $id_tkr_kode_ipwdes);
			if ($operasi == "1") {
				$ket = "ID : $id,\nKode Lahan: $jenis_ternak,\nJenis Pertanian: $jumlah_jantan,\nLuas Jumlah : $jumlah,\nKeterangan: $keterangan,\ntahun: $tahun,\nID Desa: $id_des,\nID Kecamatan: $id_kec,\nIPW Desa: $id_tkr_kode_ipwdes";
				$this->Mlog->log_history("TKR Peternakan Desa", "Update", $ket);
			}
			echo base64_encode($operasi);
		} else {
			echo base64_encode("99");
	...
	public function tambah_akses()
	{
		if ($this->aksesc[1] == "1") {
			$id = trim(str_replace("'", "''", $this->input->post("id")));
			$jenis_ternak = trim(str_replace("'", "''", $this->input->post("nama")));
			...
			$operasi = $this->Mtkrptknds123->tambah_akses($id, $jenis_ternak, $uraian, $pass, $level, $status);
			if ($operasi == "1") {
				$ket = "ID Akses: $id,\nNama: $jenis_ternak,\nuraian: $uraian,\nPassword: $pass,\nlevel: $level,\nstatus: $status";
	...
	public function hapus()
	{
		if ($this->aksesc[2] == "1") {
			$id = trim(str_replace("'", "''", $this->input->post("id")));
			$td = $this->Mtkrptknds123->cekform($id);
			if (is_array($td)) {
				if (count($td) > 0) {
					echo base64_encode("90");
				} else {
					$dt = $this->Mtkrptknds123->filter($id);
					$operasi = $this->Mtkrptknds123->hapus($id);
					if ($operasi == "1") {
						foreach ($dt as $k) {
							$id = $k->id;
							$jenis_ternak = $k->jenis_ternak;
						...
						$ket = "Kode Lahan: $jenis_ternak,\nJenis Pertanian: $jumlah_jantan,\nLuas Jumlah : $jumlah,\nKeterangan: $keterangan,\ntahun: $tahun,\nID Desa: $id_des,\nID Kecamatan: $id_kec,\nIPW Desa: $id_tkr_kode_ipwdes";
						$this->Mlog->log_history("TKR Peternakan Desa", "Hapus", $ket);
					}
					echo base64_encode($operasi);
				}
			} else {
	...
	// 	$jenis_ternak = str_replace("fileku", $kode, $filename);
	// 	$location = "assets/excel/" . $jenis_ternak;
	...
	// 					$jab = $d[22];
	// 					if ($baris != 1) {
	// 						if ($jenis_ternak == "" || $nik == "" || $nip == "" || $gelarDep == "" || $gelarBel == "" || $tempat_lahir == "" || $tgl_lahir == "" || $jumlah_jantan == "" || $agama == "" || $jsp == "" || $sek == "" || $kec == "" || $ptt == "" || $pta == "" || $jumlah == "" || $desa == "" || $pos == "" || $keterangan == "" || $email == "" || $status == "" || $jtt == "" || $jja == "" || $jab == "") {
	// 							$gagal++;
	// 						} else {

	// 							$st = $this->Mtkrptknds123->tambah(
	// 								$jenis_ternak,
	// 								$nik,
	// 								$nip,
	// 								$gelarDep,
	...
	// 		$jumlah = $k->jumlah;
	// 		$keterangan = $k->$keterangan;
	// 		array_push($arr, [$id, $jenis_ternak, $jumlah_jantan, $jumlah, $keterangan]);
	// 	}
