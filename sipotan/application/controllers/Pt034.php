...
class Pt034 extends CI_Controller {
	public $idsc;
	public $aksesc = array();
	...
	public function json(){
        ...
        foreach ($dt as $k){
        	...
        	$dtisi .= '["'.$tomboledit.'","'.$id.'","'.$nama.'","'.$tgl.'","'.$no.'","'.$status.'","'.$alamat.'","'.$gp.'","'.$telp.'","'.$email.'"],';
        }
	...
	public function filter(){
		...
		if(is_array($dt)){
			if(count($dt) > 0){
				...
				echo base64_encode("1|".$id."|".$nama."|".$tgl."|".$no."|".$status."|".$alamat."|".$gp."|".$telp."|".$email);
	...
	public function tambah(){
		if($this->aksesc[0] == "1"){
			...
			$gp = trim(str_replace("'","''",$this->input->post("gp")));
			$telp = trim(str_replace("'","''",$this->input->post("telp")));
			$email = trim(str_replace("'","''",$this->input->post("email")));
			$operasi = $this->Mpt034->tambah($nama,$tgl,$no,$status, $alamat,$gp,$telp,$email);
			if($operasi == "1"){
				$ket = "Nama Poktan: $nama,\nTanggal Berdiri: $tgl,\nNomor Legalitas: $no,\nStatus: $status,\nAlamat: $alamat,\nID Gapoktan: $gp,\nTelp: $telp,\nEmail: $email";
				$this->Mlog->log_history("Poktan","Tambah",$ket);
			}
	...
	public function update(){
		if($this->aksesc[1] == "1"){
			...
			$telp = trim(str_replace("'","''",$this->input->post("telp")));
			$email = trim(str_replace("'","''",$this->input->post("email")));
			$operasi = $this->Mpt034->update($id, $nama,$tgl,$no,$status, $alamat,$gp,$telp,$email);
			if($operasi == "1"){
				$ket = "ID Poktan: $id,\nNama Poktan: $nama,\nTanggal Berdiri: $tgl,\nNomor Legalitas: $no,\nStatus: $status,\nAlamat: $alamat,\nID Gapoktan: $gp,\nTelp: $telp,\nEmail: $email";
				$this->Mlog->log_history("Poktan","Update",$ket);
			}
			echo base64_encode($operasi);
		}else{echo base64_encode("99");}
	}
		...
		if($this->aksesc[2] == "1"){
			...
			if(is_array($td)){
				if(count($td) > 0){echo base64_encode("90");}
					...
					$dt = $this->Mpt034->filter($id);
					$operasi = $this->Mpt034->hapus($id);
					if($operasi == "1"){
						foreach ($dt as $k){
							$id = $k->id;
				            $nama = $k->nama;
				            $tgl = $k->tanggal_berdiri;
				            $no = $k->no_legalitas;
			        	  	$status = $k->status;
				            $alamat = $k->alamat;
				            $gp = $k->id_gapoktan;
						...
						$ket = "ID Poktan: $id,\nNama Poktan: $nama,\nTanggal Berdiri: $tgl,\nNomor Legalitas: $no,\nStatus: $status,\nAlamat: $alamat,\nID Gapoktan: $gp,\nTelp: $telp,\nEmail: $email";
						$this->Mlog->log_history("Poktan","Hapus",$ket);
					}
					echo base64_encode($operasi);
				}
