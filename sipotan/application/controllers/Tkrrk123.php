...
class Tkrrk123 extends CI_Controller
{
	public $idsc;
	public $aksesc = array();
	...
	public function json()
	{
		...
		foreach ($dt as $k) {
			...
			$no_urut_relasi = $k->no_urut_relasi;
			$tanggal_relasi = $k->tanggal_relasi;
			$penjelasan = $k->penjelasan;
			$alternatif = $k->alternatif;
			$bukti_fisik = $k->bukti_fisik;
			$is_tpp = $k->is_tpp;
			...
			$dtisi .= '["' . $tomboledit . '","' . $id . '","' . $bulan_ke . '","' . $tahun . '","' . $nip . '","' . $kode_jenjang . '","' . $no_urut_rencana . '","' . $pangkat_gol_asli . '","' . $no_urut_relasi . '","' . $tanggal_relasi . '","' . $penjelasan . '","' . $alternatif . '","' . $bukti_fisik . '","' . $is_tpp . '"],';
		}
	...
	public function filter()
	{
		...
		if (is_array($dt)) {
			if (count($dt) > 0) {
				...
				echo base64_encode("1|" . $id . "|" . $bulan_ke . "|" . $tahun . "|" . $nip . "|" . $kode_jenjang . "|" . $no_urut_rencana . "|" . $pangkat_gol_asli . "|"  . $no_urut_relasi . "|" . $tanggal_relasi . "|" . $penjelasan . "|" . $alternatif . "|" . $bukti_fisik . "|" . $is_tpp);
			} else {
	...
	public function filterdesa()
	{
		...
		$hasil = $this->Mtkrrk123->filterdesa($idkec);
	...
	public function tambah()
	{
		if ($this->aksesc[0] == "1") {
			...
			$no_urut_relasi = trim(str_replace("'", "''", $this->input->post("no_urut_relasi")));
			$tanggal_relasi = trim(str_replace("'", "''", $this->input->post("tanggal_relasi")));
			$penjelasan = trim(str_replace("'", "''", $this->input->post("penjelasan")));
			$alternatif = trim(str_replace("'", "''", $this->input->post("alternatif")));
			$bukti_fisik = trim(str_replace("'", "''", $this->input->post("bukti_fisik")));
			$is_tpp = trim(str_replace("'", "''", $this->input->post("is_tpp")));
			$operasi = $this->Mtkrrk123->tambah($bulan_ke, $tahun, $nip, $kode_jenjang, $no_urut_rencana, $pangkat_gol_asli, $no_urut_relasi, $tanggal_relasi, $penjelasan, $alternatif, $bukti_fisik, $is_tpp);
			if ($operasi == "1") {
				$ket = "Bulan ke: $bulan_ke,\nTahun: $tahun,\nNIP : $nip,\nkode_jenjang: $kode_jenjang,\nNo Urut Rencana: $no_urut_rencana,\nPangkat Gol Asli: $pangkat_gol_asli\nNo Urut Relasi: $no_urut_relasi,\nTanggal Relasi: $tanggal_relasi,\nPenjelasan: $penjelasan,\Alternatif: $alternatif,\nBukti Fisik: $bukti_fisik,\Is TPP: $is_tpp";
				$this->Mlog->log_history("TKR Realisasi Kegiatan", "Tambah", $ket);
			}
	...
	public function update()
	{
		if ($this->aksesc[1] == "1") {
			...
			$bukti_fisik = trim(str_replace("'", "''", $this->input->post("bukti_fisik")));
			$is_tpp = trim(str_replace("'", "''", $this->input->post("is_tpp")));
			$operasi = $this->Mtkrrk123->update($id, $bulan_ke, $tahun, $nip, $kode_jenjang, $no_urut_rencana, $pangkat_gol_asli, $no_urut_relasi, $tanggal_relasi, $penjelasan, $alternatif, $bukti_fisik, $is_tpp);
			if ($operasi == "1") {
				$ket = "Bulan ke: $bulan_ke,\nTahun: $tahun,\nNIP : $nip,\nkode_jenjang: $kode_jenjang,\nNo Urut Rencana: $no_urut_rencana,\nPangkat Golongan Asli: $pangkat_gol_asli,\nNo Urut Relasi: $no_urut_relasi,\nTanggal Relasi: $tanggal_relasi,\nPenjelasan: $penjelasan,\Alternatif: $alternatif,\nBukti Fisik: $bukti_fisik,\Is TPP: $is_tpp";
				$this->Mlog->log_history("TKR Realisasi Kegiatan", "Update", $ket);
			}
			echo base64_encode($operasi);
		} else {
			echo base64_encode("99");
	...
	public function tambah_akses()
	{
		if ($this->aksesc[1] == "1") {
			...
			$operasi = $this->Mtkrrk123->tambah_akses($id, $bulan_ke, $no_urut_rencana, $pass, $level, $status);
			if ($operasi == "1") {
	...
	public function hapus()
	{
		if ($this->aksesc[2] == "1") {
			$id = trim(str_replace("'", "''", $this->input->post("id")));
			$td = $this->Mtkrrk123->cekform($id);
			if (is_array($td)) {
				if (count($td) > 0) {
					echo base64_encode("90");
				} else {
					$dt = $this->Mtkrrk123->filter($id);
					$operasi = $this->Mtkrrk123->hapus($id);
					if ($operasi == "1") {
						foreach ($dt as $k) {
							$id = $k->id;
							$bulan_ke = $k->bulan_ke;
						...
						$ket = "ID: $id\nBulan ke: $bulan_ke,\nTahun: $tahun,\nNIP : $nip,\nkode_jenjang: $kode_jenjang,\nNo Urut Rencana: $no_urut_rencana,\nPangkat Golongan Asli: $pangkat_gol_asli,\nNo Urut Relasi: $no_urut_relasi,\nTanggal Relasi: $tanggal_relasi,\nPenjelasan: $penjelasan,\Alternatif: $alternatif,\nBukti Fisik: $bukti_fisik,\Is TPP: $is_tpp";
						$this->Mlog->log_history("TKR Realisasi Kegiatan", "Hapus", $ket);
					}
					echo base64_encode($operasi);
				}
			} else {
