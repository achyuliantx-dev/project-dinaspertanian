...
class Pk003 extends CI_Controller {
	public $idsc;
	public $aksesc = array();
	...
	public function tambah(){
		if($this->aksesc[0] == "1"){
			...
			$operasi = $this->Mpk003->tambah($nama);
			if($operasi == "1"){
				$ket = "Nama Pupuk: $nama";
				$this->Mlog->log_history("Pupuk","Tambah",$ket);
			}
	...
	public function update(){
		if($this->aksesc[1] == "1"){
			...
			$nama = trim(str_replace("'","''",$this->input->post("nama")));
			$operasi = $this->Mpk003->update($id, $nama);
			if($operasi == "1"){
				$ket = "ID Pupuk: $id,\nNama Pupuk: $nama";
				$this->Mlog->log_history("Pupuk","Update",$ket);
			}
			echo base64_encode($operasi);
		}else{echo base64_encode("99");}
	}
	...
	public function hapus(){
		if($this->aksesc[2] == "1"){
			$id = trim(str_replace("'","''",$this->input->post("id")));
			$td = $this->Mpk003->cekform($id);
			if(is_array($td)){
				if(count($td) > 0){echo base64_encode("90");}
				else{
					$dt = $this->Mpk003->filter($id);
					$operasi = $this->Mpk003->hapus($id);
					if($operasi == "1"){
						foreach ($dt as $k){
							$id = $k->id;
							$nama = $k->nama;
						...
						$ket = "ID Pupuk: $id,\nNama Pupuk: $nama";
						$this->Mlog->log_history("Pupuk","Hapus",$ket);
					}
					echo base64_encode($operasi);
				}
