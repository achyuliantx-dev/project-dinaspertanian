...
class Pt071 extends CI_Controller {
	public $idsc;
	public $aksesc = array();
	...
	public function json(){
        ...
        foreach ($dt as $k){
            ...
            $dtisi .= '["'.$tomboledit.'","'.$id.'","'.$nop.'","'.$mt.'","'.$tgl.'","'.$komoditas.'"],';
        }
	...
	public function filter(){
		...
		if(is_array($dt)){
			if(count($dt) > 0){
				...
				echo base64_encode("1|".$id."|".$nop."|".$mt."|".$tgl."|".$komoditas);
	...
	public function update(){
		if($this->aksesc[1] == "1"){
			...
			$tgl = trim(str_replace("'","''",$this->input->post("tgl")));
			$komoditas = trim(str_replace("'","''",$this->input->post("komoditas")));
			$operasi = $this->Mpt071->update($id, $nop, $mt, $tgl, $komoditas);
			if($operasi == "1"){
	...
	public function hapus(){
		if($this->aksesc[2] == "1"){
			$id = trim(str_replace("'","''",$this->input->post("id")));
			$td = $this->Mpt071->cekform($id);
			if(is_array($td)){
				if(count($td) > 0){echo base64_encode("90");}
				else{
					$dt = $this->Mpt071->filter($id);
					$operasi = $this->Mpt071->hapus($id);
					if($operasi == "1"){
						foreach ($dt as $k){
							$id = $k->id;
							$nop = $k->nop;
