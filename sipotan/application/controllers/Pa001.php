...
class Pa001 extends CI_Controller {
	public $idsc;
	public $aksesc = array();
	...
	public function json(){
        ...
        foreach ($dt as $k){
            ...
            $dtisi .= '["'.$tomboledit.'","'.$id.'","'.$kac.'","'.$ket.'","'.$sek.'","'.$ben.'","'.$tgl1.'","'.$tgl.'"],';
        }
	...
	public function filter(){
		...
		if(is_array($dt)){
			if(count($dt) > 0){
				...
				echo base64_encode("1|".$id."|".$kec."|".$ket."|".$sek."|".$ben."|".$tgl1."|".$tgl);
	...
	public function tambah(){
		if($this->aksesc[0] == "1"){
			...
			$operasi = $this->Mpa001->tambah($kec, $ket, $sek, $ben, $tgl1, $tgl);
			if($operasi == "1"){
				$ket = "Nama Asosiasi Kecamatan: $kec,\nKetua: $ket,\nSekretaris: $sek,\nBendahara: $ben,\nPeriode Awal: $tgl1,\nPeriode Akhir: $tgl";
				$this->Mlog->log_history("Pengurus Asosiasi Kecamatan","Tambah",$ket);
			}
	...
	public function update(){
		if($this->aksesc[1] == "1"){
			...
			$tgl1 = trim(str_replace("'","''",$this->input->post("tgl1")));
			$tgl = trim(str_replace("'","''",$this->input->post("tgl")));
			$operasi = $this->Mpa001->update($id, $kec, $ket, $sek, $ben, $tgl1, $tgl);
			if($operasi == "1"){
				$ket = "ID Pengurus Asosiasi Kecamatan: $id,\nNama Asosiasi Kecamatan: $kec,\nKetua: $ket,\nSekretaris: $sek,\nBendahara: $ben,\nPeriode Awal: $tgl1,\nPeriode Akhir: $tgl";
				$this->Mlog->log_history("Pengurus Asosiasi Kecamatan","Update",$ket);
			}
			echo base64_encode($operasi);
		}else{echo base64_encode("99");}
	}
	...
	public function hapus(){
		if($this->aksesc[2] == "1"){
			$id = trim(str_replace("'","''",$this->input->post("id")));
			$td = $this->Mpa001->cekform($id);
			if(is_array($td)){
				if(count($td) > 0){echo base64_encode("90");}
				else{
					$dt = $this->Mpa001->filter($id);
					$operasi = $this->Mpa001->hapus($id);
					if($operasi == "1"){
						foreach ($dt as $k){
							$id = $k->id;
				            $kec = $k->id_asosiasi_kec;
						...
						$ket = "ID Pengurus Asosiasi Kecamatan: $id,\nNama Asosiasi Kecamatan: $kec,\nKetua: $ket,\nSekretaris: $sek,\nBendahara: $ben,\nPeriode Awal: $tgl1,\nPeriode Akhir: $tgl";
				$this->Mlog->log_history("Pengurus Asosiasi Kecamatan","Hapus",$ket);
					}
					echo base64_encode($operasi);
				}
