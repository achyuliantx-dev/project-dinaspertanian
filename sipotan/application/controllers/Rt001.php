...
class Rt001 extends CI_Controller
{
	public $idsc;
	public $aksesc = array();
	...
	public function tambah()
	{
		if ($this->aksesc[0] == "1") {
			...
			$operasi = $this->Mrt001->tambah($nama);
			if ($operasi == "1") {
				$ket = "Nama Range Pangkat: $nama";
				$this->Mlog->log_history("Range Pangkat", "Tambah", $ket);
			}
	...
	public function update()
	{
		if ($this->aksesc[1] == "1") {
			...
			$nama = trim(str_replace("'", "''", $this->input->post("nama")));
			$operasi = $this->Mrt001->update($id, $nama);
			if ($operasi == "1") {
				$ket = "ID Range Pangkat: $id,\nNama Range Pangkat: $nama";
				$this->Mlog->log_history("Range Pangkat", "Update", $ket);
			}
			echo base64_encode($operasi);
		} else {
			echo base64_encode("99");
	...
	public function hapus()
	{
		if ($this->aksesc[2] == "1") {
			$id = trim(str_replace("'", "''", $this->input->post("id")));
			$td = $this->Mrt001->cekform($id);
			if (is_array($td)) {
				if (count($td) > 0) {
					echo base64_encode("90");
				} else {
					$dt = $this->Mrt001->filter($id);
					$operasi = $this->Mrt001->hapus($id);
					if ($operasi == "1") {
						foreach ($dt as $k) {
							$id = $k->id;
							$nama = $k->nama;
						...
						$ket = "ID Range Pangkat: $id,\nNama Range Pangkat: $nama";
						$this->Mlog->log_history("Range Pangkat", "Hapus", $ket);
					}
					echo base64_encode($operasi);
				}
			} else {
	...
	{
		$arr = [['<center><b><style font-size="14">DATA Range Pangkat</style></b></center>'], ['<center><b>ID</b></center>', '<center><b>Nama</b></center>']];
		$dt = $this->Mrt001->data();
		foreach ($dt as $k) {
			$id = $k->id;
			$nama = $k->nama;
		...
		$xlsx->downloadAs('Data Range Pangkat.xlsx');
	}

	public function download()
	{
		force_download('assets/excel/Format Data Range Pangkat.xlsx', NULL);
	}
}
