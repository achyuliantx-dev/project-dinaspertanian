...
class Kb844 extends CI_Controller {
	public $idsc;
	public $aksesc = array();
	...
	public function json(){
        ...
        foreach ($dt as $k){
            ...
            $dtisi .= '["'.$tomboledit.'","'.$id.'","'.$nama.'","'.$pro.'","'.$jenis.'"],';
        }
	...
	public function filter(){
		...
		if(is_array($dt)){
			if(count($dt) > 0){
				...
				echo base64_encode("1|".$id."|".$nama."|".$prov."|".$jenis);
	...
	public function tambah(){
		if($this->aksesc[0] == "1"){
			...
			$prov = trim(str_replace("'","''",$this->input->post("prov")));
			$jenis = trim(str_replace("'","''",$this->input->post("jenis")));
			$operasi = $this->Mkb844->tambah($id, $nama,$prov,$jenis);
			if($operasi == "1"){
				$ket = "ID Kabupaten: $id,\nNama Kabupaten: $nama,\nID Propinsi: $prov,\nJenis: $jenis";
				$this->Mlog->log_history("Kabupaten","Tambah",$ket);
			}
	...
	public function update(){
		if($this->aksesc[1] == "1"){
			...
			$prov = trim(str_replace("'","''",$this->input->post("prov")));
			$jenis = trim(str_replace("'","''",$this->input->post("jenis")));
			$operasi = $this->Mkb844->update($id, $nama,$prov,$jenis);
			if($operasi == "1"){
				$ket = "ID Kabupaten: $id,\nNama Kabupaten: $nama,\nID Propinsi: $prov,\nJenis: $jenis";
				$this->Mlog->log_history("Kabupaten","Update",$ket);
			}
			echo base64_encode($operasi);
		}else{echo base64_encode("99");}
	}
	...
	public function hapus(){
		if($this->aksesc[2] == "1"){
			$id = trim(str_replace("'","''",$this->input->post("id")));
			$td = $this->Mkb844->cekform($id);
			if(is_array($td)){
				if(count($td) > 0){echo base64_encode("90");}
				else{
					$dt = $this->Mkb844->filter($id);
					$operasi = $this->Mkb844->hapus($id);
					if($operasi == "1"){
						foreach ($dt as $k){
							$id = $k->id;
							$nama = $k->nama;
						...
						$ket = "ID Kabupaten: $id,\nNama Kabupaten: $nama,\nID Propinsi: $prov,\nJenis: $jenis";
						$this->Mlog->log_history("Kabupaten","Hapus",$ket);
					}
					echo base64_encode($operasi);
				}
