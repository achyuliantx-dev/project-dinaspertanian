...
class Pr012 extends CI_Controller {
	public $idsc;
	public $aksesc = array();
	...
	public function tambah(){
		if($this->aksesc[0] == "1"){
			...
			$operasi = $this->Mpr012->tambah($id, $nama);
			if($operasi == "1"){
				$ket = "ID Propinsi: $id,\nNama Propinsi: $nama";
				$this->Mlog->log_history("Propinsi","Tambah",$ket);
			}
	...
	public function update(){
		if($this->aksesc[1] == "1"){
			$id = trim(str_replace("'","''",$this->input->post("id")));
			$nama = trim(str_replace("'","''",$this->input->post("nama")));
			$operasi = $this->Mpr012->update($id, $nama);
			if($operasi == "1"){
				$ket = "ID Propinsi: $id,\nNama Propinsi: $nama";
				$this->Mlog->log_history("Propinsi","Update",$ket);
			}
			echo base64_encode($operasi);
		}else{echo base64_encode("99");}
	}
	...
	public function hapus(){
		if($this->aksesc[2] == "1"){
			$id = trim(str_replace("'","''",$this->input->post("id")));
			$td = $this->Mpr012->cekform($id);
			if(is_array($td)){
				if(count($td) > 0){echo base64_encode("90");}
				else{
					$dt = $this->Mpr012->filter($id);
					$operasi = $this->Mpr012->hapus($id);
					if($operasi == "1"){
						foreach ($dt as $k){
							$id = $k->id;
							$nama = $k->nama;
						...
						$ket = "ID Propinsi: $id,\nNama Propinsi: $nama";
						$this->Mlog->log_history("Propinsi","Hapus",$ket);
					}
					echo base64_encode($operasi);
				}
