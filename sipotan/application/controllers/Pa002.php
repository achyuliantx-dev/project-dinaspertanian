...
class Pa002 extends CI_Controller {
	public $idsc;
	public $aksesc = array();
	function __construct() {
		parent::__construct();
		$idformini = "pa002";
		$this->load->model("Mpa002");
	...
	public function tambah(){
		if($this->aksesc[0] == "1"){
			...
			$operasi = $this->Mpa002->tambah($akab, $ket, $sek, $ben, $pawal, $pakhir);
			if($operasi == "1"){
				$ket = "ID Asosiasi Kabupaten: $akab,\nKetua: $ket,\nSekretaris: $sek,\nBendahara: $ben,\nPeriode Awal: $pawal,\nPeriode Akhir: $pakhir";
				$this->Mlog->log_history("Pengurus Asosiasi Kabupaten","Tambah",$ket);
			}
	...
	public function update(){
		if($this->aksesc[1] == "1"){
			...
			$pawal = trim(str_replace("'","''",$this->input->post("pawal")));
			$pakhir = trim(str_replace("'","''",$this->input->post("pakhir")));
			$operasi = $this->Mpa002->update($id, $akab, $ket, $sek, $ben, $pawal, $pakhir);
			if($operasi == "1"){
				$ket = "ID Pengurus Asosiasi Kabupaten: $id,\nID Asosiasi Kabupaten: $akab,\nKetua: $ket,\nSekretaris: $sek,\nBendahara: $ben,\nPeriode Awal: $pawal,\nPeriode Akhir: $pakhir";
				$this->Mlog->log_history("Pengurus Asosiasi Kabupaten","Update",$ket);
			}
			echo base64_encode($operasi);
		}else{echo base64_encode("99");}
	}
	...
	public function hapus(){
		if($this->aksesc[2] == "1"){
			$id = trim(str_replace("'","''",$this->input->post("id")));
			$td = $this->Mpa002->cekform($id);
			if(is_array($td)){
				if(count($td) > 0){echo base64_encode("90");}
				else{
					$dt = $this->Mpa002->filter($id);
					$operasi = $this->Mpa002->hapus($id);
					if($operasi == "1"){
						foreach ($dt as $k){
							$id = $k->id;
				            $akab = $k->id_asosiasi_kab;
						...
						$ket = "ID Pengurus Asosiasi Kabupaten: $id,\nID Asosiasi Kabupaten: $akab,\nKetua: $ket,\nSekretaris: $sek,\nBendahara: $ben,\nPeriode Awal: $pawal,\nPeriode Akhir: $pakhir";
						$this->Mlog->log_history("Pengurus Asosiasi Kabupaten","Hapus",$ket);
					}
					echo base64_encode($operasi);
				}
