<div class="page-header">
	<h1 class="page-title">Pengelolaan Data Propinsi</h1>
			...
			<div class="panel-body">
				<div class="form-row">
                    <div class="form-group col-md-12">
						<label class="form-control-label">ID Propinsi*</label>
						<input type="text" class="form-control khusus_angka jedaobyek" id="txtid" placeholder="Masukkan Kode Propinsi" autocomplete="off" maxlength="3">
					</div>
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label">Nama Propinsi*</label>
						<input type="text" class="form-control khusus_abjad jedaobyek" id="txtnama" placeholder="Masukkan Nama Propinsi" autocomplete="off">
					</div>
...
<script>
	...
	function tambah(){
        ...
        $.ajax({
            ...
            success: function(x){
                ...
                if(y == 1){
                    swal({
						...
						text: 'Data Propinsi Berhasil di Tambahkan',
						icon: 'success'
					}).then((Refreshh)=>{
						refresh();
						tabel.ajax.reload(null, false);
			...
			error: function(){
				swal.close();
				swal({title: 'Tambah Gagal', text: 'Id yang anda masukkan sudah ada !', icon: 'error'});
			}
		})
		$("#btntambah").attr("disabled", false);
	}
            ...
            success: function(x){
                ...
                if(y == 1){
                    swal({
						...
						text: 'Data Propinsi Berhasil di Update',
						icon: 'success'
					}).then((Refreshh)=>{
						refresh();
						tabel.ajax.reload(null, false);
	...
	function hapus(){
        ...
        if(id == ""){
            swal({title: 'Hapus Gagal', text: 'ID Propinsi Kosong !', icon: 'error'});
			$("#btnupdate").attr("disabled", false);
			$("#btnhapus").attr("disabled", false);
			$("#btnrefresh").attr("disabled", false);
            return;
					...
					success: function(x){
						...
						if(y == 1){
							swal({
								...
								text: 'Data Propinsi Berhasil di Hapus',
								icon: 'success'
							}).then((Refreshh)=>{
								refresh();
								tabel.ajax.reload(null, false);
							...
							if(y == 99){
								...
								if(y == 90){
									swal({title: 'Hapus Gagal', text: 'Data Propinsi Ini Masih digunakan dalam Data Kabupaten, Sehingga Tidak Dapat di Hapus Hanya Dapat di Ubah', icon: 'error'});
									refresh();
								}else{
