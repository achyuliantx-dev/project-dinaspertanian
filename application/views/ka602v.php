<div class="page-header">
	<h1 class="page-title">Pengurus Asosiasi Kabupaten</h1>
			...
			<div class="panel-body">
				<div class="form-row">
                     <div class="form-group col-md-12">
						<label class="form-control-label">ID Pengurus Asosiasi Kab*</label>
					...
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label" style="margin-bottom: -5px;">Asosiasi Kab</label>
						<select class="form-control select2" id="cboak">
					...
					<div class="form-group col-md-12">
						...
						<input type="text" class="form-control khusus_abjad jedaobyek" id="txtket" placeholder="Masukkan Nama Ketua" autocomplete="off">
					</div>
					<div class="form-group col-md-12">
						<label class="form-control-label">Sekretaris*</label>
						<input type="text" class="form-control khusus_abjad jedaobyek" id="txtsek" placeholder="Masukkan Nama Sekretaris" autocomplete="off">
					</div>
					<div class="form-group col-md-12">
						<label class="form-control-label">Bendahara*</label>
						<input type="text" class="form-control khusus_abjad jedaobyek" id="txtben" placeholder="Masukkan Nama Bendahara" autocomplete="off">
					</div>
					<div class="form-group col-md-12">
						<label class="form-control-label">Periode Awal*</label>
						<input type="date" class="form-control khusus_abjad jedaobyek" id="txtawal" placeholder="Masukkan Periode Awal" autocomplete="off">
					</div>
					<div class="form-group col-md-12">
						<label class="form-control-label">Periode Akhir*</label>
						<input type="date" class="form-control khusus_abjad jedaobyek" id="txtakhir" placeholder="Masukkan Periode Akhir" autocomplete="off">
					</div>
...
<script>
	...
	function tambah(){
        ...
        var id = $("#txtid").val();
        var akab = $("#cboak").val();
        var ketua = $("#txtket").val();
		var sekretaris = $("#txtsek").val();
		var bendahara = $("#txtben").val();
		var pawal = $("#txtawal").val();
		var pakhir = $("#txtakhir").val();

        if(id == "" || akab == "" || ketua == "" || sekretaris == "" || bendahara == "" || pawal == "" || pakhir == ""){
        ...
        $.ajax({
            ...
            method: "POST",
            data: {id: id, akab: akab, ketua: ketua, sekretaris: sekretaris, bendahara: bendahara, pawal: pawal, pakhir: pakhir},
            cache: "false",
            success: function(x){
				swal.close();
				var y = atob(x);
                if(y == 1){
                    swal({
						...
						text: 'Data Pengurus Asosiasi Kab Berhasil di Tambahkan',
						icon: 'success'
					}).then((Refreshh)=>{
						refresh();
						tabel.ajax.reload(null, false);
            ...
            success: function(x){
                ...
                if(y == 1){
                    swal({
						...
						text: 'Data Pengurus Asosiasi Kab Berhasil di Update',
						icon: 'success'
					}).then((Refreshh)=>{
						refresh();
						tabel.ajax.reload(null, false);
	...
	function hapus(){
        ...
        if(id == ""){
            swal({title: 'Hapus Gagal', text: 'ID Rencana PT Kosong !', icon: 'error'});
            return;
		}
		swal("Memproses Data.....", {button: false, closeOnClickOutside: false, closeOnEsc: false});
		swal({
					...
					success: function(x){
						...
						if(y == 1){
							swal({
								...
								text: 'Data Pengurus Asosiasi Kab Berhasil di Hapus',
								icon: 'success'
							}).then((Refreshh)=>{
								refresh();
								tabel.ajax.reload(null, false);
