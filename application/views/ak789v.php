<div class="page-header">
	<h1 class="page-title">Pengelolaan Data Asosiasi Kabupaten</h1>
			...
			<div class="panel-body">
				<div class="form-row">
                     <div class="form-group col-md-12">
						<label class="form-control-label">ID Asosiasi Kbupaten*</label>
					 ...
					 <div class="form-group col-md-12">
						...
						<input type="text" class="form-control khusus_abjad jedaobyek" id="txtnama" placeholder="Masukkan Nama Asosiasi Kab" autocomplete="off" maxlength="100">
					</div>
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label">Tanggal Berdiri*</label>
						<input type="date" class="form-control khusus_abjad jedaobyek" id="txttgl" placeholder="Masukkan Tanggal Berdiri" autocomplete="off" maxlength="100">
					</div>
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label">No Legalitas*</label>
						<input type="text" class="form-control jedaobyek" id="txtlegalitas" placeholder="Masukkan No Legalitas" autocomplete="off" maxlength="100">
					</div>
					<div class="form-group col-md-12 jedaobyek">
					...
					<div class="form-group col-md-12 jedaobyek">
						...
						<input type="text" class="form-control khusus_abjad jedaobyek" id="txtalamat" placeholder="Masukkan Alamat" autocomplete="off" maxlength="100">
					</div>
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label" style="margin-bottom: -5px;">Kabupaten</label>
						<select class="form-control select2" id="cbokab">
					...
					<div class="form-group col-md-12 jedaobyek">
						...
						<input type="text" class="form-control khusus_abjad jedaobyek" id="txttelepon" placeholder="Masukkan Telepon" autocomplete="off" maxlength="100">
					</div>
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label">Email*</label>
						<input type="text" class="form-control jedaobyek" id="txtemail" placeholder="Masukkan Email" autocomplete="off" maxlength="100">
					</div>
...
<script>
	...
	function tambah(){
		...
		var tgl_ber = $("#txttgl").val();
		var no_leg = $("#txtlegalitas").val();
		var status = $("#cbostatus").val();
		var alamat = $("#txtalamat").val();
		var kabu = $("#cbokab").val();
		var telp = $("#txttelepon").val();
		var email = $("#txtemail").val();
		
       if(id == "" || nama == "" || tgl_ber == ""|| no_leg == "" || status == "" || alamat == "" || kabu == "" ||  telp == "" || email == ""){
        ...
        $.ajax({
            ...
            method: "POST",
            data: {id: id, nama: nama, tgl_ber: tgl_ber, no_leg: no_leg, status: status, alamat: alamat, kabu: kabu, telp: telp, email: email},
            cache: "false",
            success: function(x){
				swal.close();
				var y = atob(x);
                if(y == 1){
                    swal({
						...
						text: 'Data Asosiasi Kabupaten Berhasil di Tambahkan',
						icon: 'success'
					}).then((Refreshh)=>{
						refresh();
						tabel.ajax.reload(null, false);
            ...
            success: function(x){
                ...
                if(y == 1){
                    swal({
						...
						text: 'Data Asosiasi Kabupaten Berhasil di Update',
						icon: 'success'
					}).then((Refreshh)=>{
						refresh();
						tabel.ajax.reload(null, false);
					...
					success: function(x){
						...
						if(y == 1){
							swal({
								...
								text: 'Data Asosiasi Kabupaten Berhasil di Hapus',
								icon: 'success'
							}).then((Refreshh)=>{
								refresh();
								tabel.ajax.reload(null, false);
		...
		$.ajax({
            ...
            success: function(x){
				...
				if(xx[0] == 1){
					...
					$("#txtnama").val(xx[2]);
					$("#txttgl").val(xx[3]);
					$("#txtlegalitas").val(xx[4]);
					$("#cbostatus").val(xx[5]).change();
					$("#txtalamat").val(xx[6]);
					$("#cbokab").val(xx[7]).change();
					$("#txttelepon").val(xx[8]);
					$("#txtemail").val(xx[9]);
