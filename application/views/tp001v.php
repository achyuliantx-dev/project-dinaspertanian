<div class="page-header">
	<h1 class="page-title">Pengelolaan Data Tupoksi Golongan</h1>
			...
			<div class="panel-body">
				<div class="form-row">
					<div class="form-group col-md-12">
						<label class="form-control-label">ID Tupoksi Golongan*</label>
					...
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label" style="margin-bottom: -5px;">Jenjang</label>
						<select class="form-control select2" id="cbokec">
							<option value="">Silahkan Pilih</option>
							<?php
							if (is_array($dtjenjang)) {
								if (count($dtjenjang) > 0) {
									foreach ($dtjenjang as $x) {
					...
					<div class="form-group col-md-12 jedaobyek">
						...
						<input type="text" class="form-control khusus_abjad jedaobyek" id="txtket" placeholder="Masukkan Keterangan" autocomplete="off">
					</div>
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label" style="margin-bottom: -5px;">Range Pangkat</label>
						<select class="form-control select2" id="cbobuk">
							<option value="">Silahkan Pilih</option>
							<?php
							if (is_array($dtrange)) {
								if (count($dtrange) > 0) {
									foreach ($dtrange as $x) {
										echo "<option value='" . $x->id . "'>" . $x->nama . "</option>";
									}
								}
							}
					...
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label">Kelompok*</label>
						<input type="text" class="form-control khusus_abjad jedaobyek" id="txtkel" placeholder="Masukkan Nama" autocomplete="off">
					</div>
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label">Kuantitas*</label>
						<input type="text" class="form-control khusus_abjad jedaobyek" id="txtkun" placeholder="Masukkan Jumlah Kuantitas" autocomplete="off">
					</div>
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label">AK*</label>
						<input type="text" class="form-control khusus_abjad jedaobyek" id="txtak" placeholder="Masukkan Jumlah AK" autocomplete="off">
					</div>
...
<script>
	...
	function tambah() {
		...
		var id = $("#txtid").val();
		var jenjang = $("#cbokec").val();
		var nama = $("#txtnama").val();
		var kete = $("#txtket").val();
		var pangkat = $("#cbobuk").val();
		var kel = $("#txtkel").val();
		var kuant = $("#txtkun").val();
		var ak = $("#txtak").val();
		
		if (id == "" || jenjang == "" || nama == ""  ||pangkat == "" || kuant == ""|| ak == "" ) {
			swal({
				title: 'Tambah Gagal',
			...
			success: function(x) {
				...
				if (y == 1) {
					swal({
						...
						text: 'Data Tupoksi Golongan Berhasil di Tambahkan',
						icon: 'success'
					}).then((Refreshh) => {
						refresh();
						tabel.ajax.reload(null, false);
			...
			success: function(x) {
				...
				if (y == 1) {
					swal({
						...
						text: 'Data Tupoksi Golongan Berhasil di Update',
						icon: 'success'
					}).then((Refreshh) => {
						refresh();
						tabel.ajax.reload(null, false);
					...
					success: function(x) {
						...
						if (y == 1) {
							swal({
								...
								text: 'Data Tupoksi Golongan Berhasil di Hapus',
								icon: 'success'
							}).then((Refreshh) => {
								refresh();
								tabel.ajax.reload(null, false);
