...
class Va093 extends CI_Controller {
	...
	public function json(){
        ...
        foreach ($dt as $k){
            $id = $k->id;
            $komoditas = $k->jenis_tanaman;
            $nama = $k->nama;
            $lama_tanam = $k->lama_tanam;
            ...
            $dtisi .= '["'.$tomboledit.'","'.$id.'","'.$komoditas.'","'.$nama.'","'.$lama_tanam.'"],';
        }
	...
	public function filter(){
		...
		if(is_array($dt)){
			if(count($dt) > 0){
				foreach ($dt as $k){
					$id = $k->id;
					$komoditas = $k->id_jenis_tanaman;
					$nama = $k->nama;	
					$lama_tanam = $k->lama_tanam;
				}
				echo base64_encode("1|".$id."|".$komoditas."|".$nama."|".$lama_tanam);
	...
	public function tambah(){
		if($this->aksesc[0] == "1"){
			...
			$lama_tanam = trim(str_replace("'","''",$this->input->post("lama_tanam")));
			$operasi = $this->Mva093->tambah($komoditas, $nama, $lama_tanam);
			if($operasi == "1"){
				$ket = "ID Komoditas: $komoditas,\nNama Varietas: $nama,\nLama Tanam: $lama_tanam";
	...
	public function update(){
		if($this->aksesc[1] == "1"){
			...
			$nama = trim(str_replace("'","''",$this->input->post("nama")));
			$lama_tanam = trim(str_replace("'","''",$this->input->post("lama_tanam")));
			$operasi = $this->Mva093->update($id, $komoditas, $nama, $lama_tanam);
			if($operasi == "1"){
