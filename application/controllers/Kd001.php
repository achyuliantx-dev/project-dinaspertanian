...
class Kd001 extends CI_Controller {
	public $idsc;
	public $aksesc = array();
	...
	public function tambah(){
		if($this->aksesc[0] == "1"){
			...
			$operasi = $this->Mkd001->tambah($nama);
			if($operasi == "1"){
				$ket = "Nama Tanaman: $nama";
				$this->Mlog->log_history("Jenis Tanaman","Tambah",$ket);
			}
	...
	public function update(){
		if($this->aksesc[1] == "1"){
			...
			$nama = trim(str_replace("'","''",$this->input->post("nama")));
			$operasi = $this->Mkd001->update($id,$nama);
			if($operasi == "1"){
				$ket = "ID Tanaman: $id,\nNama Tanaman: $nama";
				$this->Mlog->log_history("Jenis Tanaman","Update",$ket);
			}
			echo base64_encode($operasi);
		}else{echo base64_encode("99");}
	}
	...
	public function hapus(){
		if($this->aksesc[2] == "1"){
			$id = trim(str_replace("'","''",$this->input->post("id")));
			$td = $this->Mkd001->cekform($id);
			if(is_array($td)){
				if(count($td) > 0){echo base64_encode("90");}
				else{
					$dt = $this->Mkd001->filter($id);
					$operasi = $this->Mkd001->hapus($id);
					if($operasi == "1"){
						foreach ($dt as $k){
							$id = $k->id;
							$nama = $k->nama;	
						...
						$ket = "ID Tanaman: $id,\nNama Tanaman: $nama";
						$this->Mlog->log_history("Jenis Tanaman","Hapus",$ket);
					}
					echo base64_encode($operasi);
				}
