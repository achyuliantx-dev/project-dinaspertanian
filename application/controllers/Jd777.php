...
class Jd777 extends CI_Controller {
	public $idsc;
	public $aksesc = array();
	...
	public function tambah(){
		if($this->aksesc[0] == "1"){
			...
			$operasi = $this->Mjd777->tambah($nama);
			if($operasi == "1"){
				$ket = "Nama Dekomposer $nama";
	...
	public function update(){
		if($this->aksesc[1] == "1"){
			...
			$nama = trim(str_replace("'","''",$this->input->post("nama")));
			$operasi = $this->Mjd777->update($id,$nama);
			if($operasi == "1"){
				$ket = "ID Dekomposer: $id,\nNama Dekomposer: $nama";
	...
	public function hapus(){
		if($this->aksesc[2] == "1"){
			$id = trim(str_replace("'","''",$this->input->post("id")));
			$td = $this->Mjd777->cekform($id);
			if(is_array($td)){
				if(count($td) > 0){echo base64_encode("90");}
				else{
					$dt = $this->Mjd777->filter($id);
					$operasi = $this->Mjd777->hapus($id);
					if($operasi == "1"){
						foreach ($dt as $k){
							$id = $k->id;
							$nama = $k->nama;	
