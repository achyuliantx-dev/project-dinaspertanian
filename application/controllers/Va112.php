...
class Va112 extends CI_Controller {
	public $idsc;
	public $aksesc = array();
	...
	public function tambah(){
		if($this->aksesc[0] == "1"){
			...
			$operasi = $this->Mva112->tambah($nama);
			if($operasi == "1"){
				$ket = "Nama Varietas: $nama";
				$this->Mlog->log_history("Varietas","Tambah",$ket);
			}
	...
	public function update(){
		if($this->aksesc[1] == "1"){
			...
			$nama = trim(str_replace("'","''",$this->input->post("nama")));
			$operasi = $this->Mva112->update($id,$nama);
			if($operasi == "1"){
				$ket = "ID Varietas: $id,\nNama Varietas: $nama";
				$this->Mlog->log_history("Varietas","Update",$ket);
			}
			echo base64_encode($operasi);
		}else{echo base64_encode("99");}
	}
	...
	public function hapus(){
		if($this->aksesc[2] == "1"){
			$id = trim(str_replace("'","''",$this->input->post("id")));
			$td = $this->Mva112->cekform($id);
			if(is_array($td)){
				if(count($td) > 0){echo base64_encode("90");}
				else{
					$dt = $this->Mva112->filter($id);
					$operasi = $this->Mva112->hapus($id);
					if($operasi == "1"){
						foreach ($dt as $k){
							$id = $k->id_varietas;
							$nama = $k->nama_varietas;	
						...
						$ket = "ID Varietas: $id,\nNama Varietas: $nama";
						$this->Mlog->log_history("Varietas","Hapus",$ket);
					}
					echo base64_encode($operasi);
				}
