...
class Op112 extends CI_Controller {
	public $idsc;
	public $aksesc = array();
	...
	public function tambah(){
		if($this->aksesc[0] == "1"){
			...
			$operasi = $this->Mop112->tambah($nama);
			if($operasi == "1"){
				$ket = "Nama OPT: $nama";
				$this->Mlog->log_history("OPT","Tambah",$ket);
			}
	...
	public function update(){
		if($this->aksesc[1] == "1"){
			...
			$nama = trim(str_replace("'","''",$this->input->post("nama")));
			$operasi = $this->Mop112->update($id,$nama);
			if($operasi == "1"){
				$ket = "ID OPT: $id,\nNama OPT: $nama";
				$this->Mlog->log_history("OPT","Update",$ket);
			}
			echo base64_encode($operasi);
		}else{echo base64_encode("99");}
	}
	...
	public function hapus(){
		if($this->aksesc[2] == "1"){
			$id = trim(str_replace("'","''",$this->input->post("id")));
			$td = $this->Mop112->cekform($id);
			if(is_array($td)){
				if(count($td) > 0){echo base64_encode("90");}
				else{
					$dt = $this->Mop112->filter($id);
					$operasi = $this->Mop112->hapus($id);
					if($operasi == "1"){
						foreach ($dt as $k){
							$id = $k->id_opt;
							$nama = $k->nama_opt;	
						...
						$ket = "ID OPT: $id,\nNama OPT: $nama";
						$this->Mlog->log_history("OPT","Hapus",$ket);
					}
					echo base64_encode($operasi);
				}
