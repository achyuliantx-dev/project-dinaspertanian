...
class Ip001 extends CI_Controller
{
	public $idsc;
	public $aksesc = array();
	...
	public function json()
	{
		...
		foreach ($dt as $k) {
			...
			$dtisi .= '["' . $tomboledit . '","' . $id . '","' . $kom . '","' . $tujuan . '","' . $rantai . '","' . $kec . '","' . $ipw . '","' . $thn . '"],';
		}
	...
	public function filter()
	{
		...
		if (is_array($dt)) {
			if (count($dt) > 0) {
				...
				echo base64_encode("1|" . $id . "|" . $komo . "|" . $tujuan . "|" . $ran. "|" . $cam . "|" . $ipwk . "|" . $thna);
			} else {
	...
	{
		if ($this->aksesc[0] == "1") {
			$komo = trim(str_replace("'", "''", $this->input->post("komo")));
			$tujuan = trim(str_replace("'", "''", $this->input->post("tujuan")));
			$ran = trim(str_replace("'", "''", $this->input->post("ran")));
			$cam = trim(str_replace("'", "''", $this->input->post("cam")));
			$ipw = trim(str_replace("'", "''", $this->input->post("ipw")));
			$thn = trim(str_replace("'", "''", $this->input->post("thn")));
			$operasi = $this->Mip001->tambah($komo, $tujuan, $ran, $cam, $ipw, $thn);
			if ($operasi == "1") {
				$ket = "Id komoditas: $komo,\nTujuan Pasar: $ujuan,\nId Rantai Pasar: $ran,\nId Kecamatan: $cam,\nId IPW Kecamatan: $ipw,\nTahun Anggaran: $thn";
				$this->Mlog->log_history("Informasi Pasar", "Tambah", $ket);
			}
	...
	public function update(){
		if ($this->aksesc[1] == "1") {
			...
			$ipw = trim(str_replace("'", "''", $this->input->post("ipw")));
			$thn = trim(str_replace("'", "''", $this->input->post("thn")));
			$operasi = $this->Mip001->update($id, $komo, $tujuan, $ran, $cam, $ipw, $thn);
			if ($operasi == "1") {
				$ket = "Id Informasi Pasar: $id,\nId komoditas: $komo,\nTujuan: $ujuan,\nId Rantai Pasar: $ran,\nId Kecamatan: $cam,\nId IPW Kecamatan: $ipw,\nTahun Anggaran: $thn";
				$this->Mlog->log_history("Informasi Pasar", "Update", $ket);
			}
			echo base64_encode($operasi);
	...
	public function hapus()
	{
		if ($this->aksesc[2] == "1") {
			$id = trim(str_replace("'", "''", $this->input->post("id")));
			$td = $this->Mip001->cekform($id);
			if (is_array($td)) {
				if (count($td) > 0) {
					echo base64_encode("90");
				} else {
					$dt = $this->Mip001->filter($id);
					$operasi = $this->Mip001->hapus($id);
					if ($operasi == "1") {
						foreach ($dt as $k) {
							$id = $k->id;
							$komo = $k->id_komoditas;
						...
						$ket = "Id Informasi Pasar: $id,\nId komoditas: $komo,\nTujuan: $ujuan,\nId Rantai Pasar: $ran,\nId Kecamatan: $cam,\nId IPW Kecamatan: $ipw,\nTahun Anggaran: $thn";
						$this->Mlog->log_history("Informasi Pasar", "Hapus", $ket);
					}
					echo base64_encode($operasi);
				}
			} else {
