...
class Kl412 extends CI_Controller {
	public $idsc;
	public $aksesc = array();
	...
	public function tambah(){
		if($this->aksesc[0] == "1"){
			...
			$operasi = $this->Mkl412->tambah($nama);
			if($operasi == "1"){
				$ket = "Jenis Lahan: $nama";
				$this->Mlog->log_history("Kategori Lahan","Tambah",$ket);
			}
	...
	public function update(){
		if($this->aksesc[1] == "1"){
			...
			$nama = trim(str_replace("'","''",$this->input->post("nama")));
			$operasi = $this->Mkl412->update($id, $nama);
			if($operasi == "1"){
				$ket = "ID Kategori Lahan: $id,\nJenis Lahan: $nama";
				$this->Mlog->log_history("Kategori Lahan","Update",$ket);
			}
			echo base64_encode($operasi);
		}else{echo base64_encode("99");}
	}
	...
	public function hapus(){
		if($this->aksesc[2] == "1"){
			$id = trim(str_replace("'","''",$this->input->post("id")));
			$td = $this->Mkl412->cekform($id);
			if(is_array($td)){
				if(count($td) > 0){echo base64_encode("90");}
				else{
					$dt = $this->Mkl412->filter($id);
					$operasi = $this->Mkl412->hapus($id);
					if($operasi == "1"){
						foreach ($dt as $k){
							$id = $k->id;
							$nama = $k->nama;
						...
						$ket = "ID Kategori Lahan: $id,\nJenis Lahan: $nama";
				$this->Mlog->log_history("Kategori Lahan","Hapus",$ket);
					}
					echo base64_encode($operasi);
				}
