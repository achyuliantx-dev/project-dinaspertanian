...
class Al111 extends CI_Controller {
	public $idsc;
	public $aksesc = array();
	...
	public function tambah(){
		if($this->aksesc[0] == "1"){
			...
			$operasi = $this->Mal111->tambah($jenis);
			if($operasi == "1"){
				$ket = "Jenis Alsintan: $jenis";
				$this->Mlog->log_history("Alsintan","Tambah",$ket);
			}
	...
	public function update(){
		if($this->aksesc[1] == "1"){
			...
			$jenis = trim(str_replace("'","''",$this->input->post("jenis")));
			$operasi = $this->Mal111->update($id,$jenis);
			if($operasi == "1"){
				$ket = "ID Alsintan: $id,\nJenis Alsintan: $jenis";
				$this->Mlog->log_history("Alsintan","Update",$ket);
			}
			echo base64_encode($operasi);
		}else{echo base64_encode("99");}
	}
	...
	public function hapus(){
		if($this->aksesc[2] == "1"){
			$id = trim(str_replace("'","''",$this->input->post("id")));
			$td = $this->Mal111->cekform($id);
			if(is_array($td)){
				if(count($td) > 0){echo base64_encode("90");}
				else{
					$dt = $this->Mal111->filter($id);
					$operasi = $this->Mal111->hapus($id);
					if($operasi == "1"){
						foreach ($dt as $k){
							$id = $k->id;
							$jenis = $k->jenis;	
						...
						$ket = "ID Alsintan: $id,\nJenis Alsintan: $jenis";
						$this->Mlog->log_history("Alsintan","Hapus",$ket);
					}
					echo base64_encode($operasi);
				}
