...
class Sk123 extends CI_Controller {
	public $idsc;
	public $aksesc = array();
	function __construct() {
		parent::__construct();
		$idformini = "sk123";
		$this->load->model("Msk123");
	...
	public function tambah(){
		if($this->aksesc[0] == "1"){
			...
			$operasi = $this->Msk123->tambah($nama);
			if($operasi == "1"){
				$ket = "Nama Sub Kegiatan: $nama";
				$this->Mlog->log_history("Sub Kegiatan","Tambah",$ket);
			}
	...
	public function update(){
		if($this->aksesc[1] == "1"){
			...
			$nama = trim(str_replace("'","''",$this->input->post("nama")));
			$operasi = $this->Msk123->update($id,$nama);
			if($operasi == "1"){
				$ket = "ID Sub Kegiatan: $id,\nNama Sub Kegiatan: $nama";
				$this->Mlog->log_history("Sub Kegiatan","Update",$ket);
			}
			echo base64_encode($operasi);
		}else{echo base64_encode("99");}
	}
	...
	public function hapus(){
		if($this->aksesc[2] == "1"){
			$id = trim(str_replace("'","''",$this->input->post("id")));
			$td = $this->Msk123->cekform($id);
			if(is_array($td)){
				if(count($td) > 0){echo base64_encode("90");}
				else{
					$dt = $this->Msk123->filter($id);
					$operasi = $this->Msk123->hapus($id);
					if($operasi == "1"){
						foreach ($dt as $k){
							$id = $k->id;
							$nama = $k->nama;	
						...
						$ket = "ID Sub Kegiatan: $id,\nNama Sub Kegiatan: $nama";
						$this->Mlog->log_history("Sub Kegiatan","Hapus",$ket);
					}
					echo base64_encode($operasi);
				}
